"""Plex backgrounds — color/vignette style.
...
if __name__ == "__main__":
    ...
    # Default to "color" but honour BACKGROUND_STYLE env var if set
    if not os.getenv("BACKGROUND_STYLE"):
        shared.background_style = "color"

    cfg = PlexConfig.from_env()
    # Optional overrides:
    # cfg.order_by = "mix"         # 'aired', 'added', or 'mix'
    # cfg.download_movies = True
    # cfg.download_series = True
    # cfg.limit = 10
    # cfg.logo_variant = "white"   # "white" or "color"

    yaml_cfg = _load_yaml(os.path.join(source_dir, "config/config.yaml"))
    _apply(shared, yaml_cfg, "shared")
    _apply(cfg, yaml_cfg, "plex")

    PlexGenerator(cfg, shared, make_renderer(shared), make_font_manager(shared), source_dir).run()
