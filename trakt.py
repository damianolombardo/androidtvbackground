"""Trakt list backgrounds — overlay style.
...
if __name__ == "__main__":
    ...
    cfg = TraktConfig.from_env()

    yaml_cfg = _load_yaml(os.path.join(source_dir, "config/config.yaml"))
    _apply(shared, yaml_cfg, "shared")
    _apply(cfg, yaml_cfg, "trakt")

    TraktGenerator(cfg, shared, make_renderer(shared), make_font_manager(shared), source_dir).run()
